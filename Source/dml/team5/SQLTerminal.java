...
/**
 * <p>
 * This class contains a sophisticated console driver to interact with the user
...
public class SQLTerminal {
	/**
	 * <p>
	 * Sophisticated console-based driver which interacts with the user and
					...
					case "setup":
						...
						do {
							...
							if (!command.isEmpty()) {
								...
								case "get":
									System.out.println(
											"\tdatabaseString\t- " + Utility.padRight(ServerSettings.getDatabase(), 30)
													+ " - the name of the database with which to connect.");
									System.out.println(
											"\tpasswordString\t- " + Utility.padRight(ServerSettings.getPassword(), 30)
													+ " - the password that corresponds to the provided username.");
									System.out.println("\tportString\t- "
											+ Utility.padRight(ServerSettings.getPort(), 30)
											+ " - the port on which the database server is listening for connections.");
									System.out.println(
											"\tserverString\t- " + Utility.padRight(ServerSettings.getServer(), 30)
													+ " - the domain or IP address belonging to the database server.");
									System.out.println(
											"\tusernameString\t- " + Utility.padRight(ServerSettings.getDatabase(), 30)
													+ " - the name which uniquely identifies each user account.");
									System.out.println("\tisOracle\t- "
											+ Utility.padRight(Boolean.toString(isOracle), 30)
											+ " - true to connect to an Oracle database, false to connect to a MySQL database.");
									System.out.println("\tisDebugMode\t- "
											+ Utility.padRight(Boolean.toString(isDebugMode), 30)
											+ " - true to print the SQL table in addition to the XML tags, false otherwise.");
									System.out.println("\tisDTD\t\t- " + Utility.padRight(Boolean.toString(isDTD), 30)
